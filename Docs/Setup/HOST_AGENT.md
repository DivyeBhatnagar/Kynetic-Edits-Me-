...
The **Kynetic Host Agent** (`host_agent/`) is a lightweight daemon installed on hardware provider machines. It probes host hardware, runs PyTorch verification benchmarks, manages Firecracker MicroVMs, handles ephemeral LUKS2 disk encryption, listens on gRPC control channels, and maintains zero-trust security isolation.
...
│  - Provisioning Diagnostics Collector (host_agent/diagnostics.py)                │
...
## 4. Control-Plane Command Channel & Diagnostics (Phases 29 & 31)

### gRPC Command Servicer (`host_agent/command_listener.py`)
Exposes typed gRPC endpoints over mutual TLS (`proto/host_agent.proto`):
- `Launch`: Initializes microVM runtime, passes WireGuard configuration, and starts workload container.
- `Stop`: Gracefully suspends VM execution.
- `Terminate`: Tears down container/VM and triggers volume shredding.
- `StreamStatus`: Streams real-time CPU/GPU utilization telemetry back to control plane.

### Idempotency Store (`host_agent/idempotency_store.py`)
Thread-safe in-memory ring buffer (capacity 1,000 keys) for command deduplication:
- Prevents duplicate workload launches if a control-plane command is re-sent due to network timeouts.
- Replayed requests return `"Already processed (idempotent replay)"` immediately without re-executing workload logic.

### Provisioning Diagnostics (`host_agent/diagnostics.py`)
- `JobDiagnostics`: Tracks timing metrics across `image_pull_ms`, `volume_setup_ms`, and `boot_time_ms`.
- Captures failure stack traces and diagnostic context for rapid troubleshooting.
