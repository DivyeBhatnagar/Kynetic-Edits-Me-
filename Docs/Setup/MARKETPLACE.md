...
This document details the Marketplace scheduling pipeline, pre-flight validation gate, intent-based AI Resource Router & Copilot, scikit-learn auto-pricing model, and transparent fallback recommendation engine.
...
## 4. Pre-Flight Validation Gate & Reservation Holds (Phase 27)

Before scheduling any instance launch request, the platform executes 4 pre-flight validation checks (`validators.py`):
1. **Requester Authorization**: Asserts `requester_id == developer_id` or requester holds an `admin` role.
2. **Listing Status**: Asserts `listing.status == active` (not paused or delisted).
3. **Host Node Status & Heartbeat Freshness**: Asserts `host.status` is listed/verified and latest `host_heartbeats.recorded_at` is within 60 seconds (`status != offline`).
4. **Developer Wallet Balance**: Calculates required hold amount ($\text{hold\_amount} = \text{price\_per\_hour} \times \text{requested\_hours}$) and verifies `wallet.balance >= hold_amount`. Rejects underfunded requests with `402 Payment Required` before contacting host agents.
