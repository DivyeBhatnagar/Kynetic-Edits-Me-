"""
Phase 1 core user models: User, UserRole, RefreshToken, PhoneOTP, AuditLog.
# ponytail: merged from models.py -> user_models.py for domain modularity
"""
...
class ApiToken(Base):
    ...
    scope: Mapped[str] = mapped_column(String(255), nullable=False, default="full")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
...
class DeviceCode(Base):
    ...
    status: Mapped[DeviceCodeStatus] = mapped_column(
        Enum(DeviceCodeStatus, name="device_code_status"),
        nullable=False,
